...
* On a existing bug, if no teamwork taskid exists on it, a new teamwork task is created, the comments and time spent gets transferred over as changes are saved, including who commented. Only new time spent gets transferred to teamwork, bugzilla does not track time at a per event level, as teamwork does. Priority transfers over and updates

This extension currently does not transfer anything else, like status changes, or bug closing or deleting. Thats for another version to come.
